...
#### 4.1.1 张量 ([具体笔记](./data/深度学习/torch_basic_tensor.ipynb))<p id="tensor中的笔记绝大多数均为天池的原文件，只是根据个人理解进行了些许更改"> 
...
#### 4.1.2 反向传播([具体笔记](./data/深度学习/torch_basic_backward.ipynb))
...
#### 4.1.3 神经网络([具体笔记](./data/深度学习/torch_basic_neural_network.ipynb))<p id="neural_network中的笔记绝大多数均为天池的原文件，只是根据个人理解进行了些许更改"> 
##### 4.1.3.1 softmax回归
...
#### 4.2.1 数据增强([具体笔记](./data/深度学习/torch_strengthen_Transform.ipynb))
数据增强本质上就是将原始的训练数据通过一系列的操作来进行变换，从而形成新的数据以参与训练。


#### 4.2.2 迁移学习([具体笔记](./data/深度学习/torch_strengthen_TransferLearning.ipynb))
...
#### 4.2.3 保存和加载模型([具体笔记](./data/深度学习/torch_strengthen_SaveAndLoad.ipynb))
最为简单的方式：
```python
# 保存  （保存可学习参数）
PATH = './download'
...
### 4.3 nn部分
#### 4.3.1 模型构建部分
- `torch.nn.Conv2d(num_inputs,32,3,stride = 2 ,padding =1)` : 创建  的卷积层
- `torch.nn.Linear(326*6,512)` ：创建  的线性层
